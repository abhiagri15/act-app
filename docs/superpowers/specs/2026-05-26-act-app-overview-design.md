...
PK: `(section, raw_score)`. **v1 seed:** a smooth linear-interpolation approximation per section (raw 0 → scaled 1, raw N_max → scaled 36) — sufficient to exercise the round-trip end-to-end and produce a sensible score band; not calibrated to any specific published form. **v2 (live, `supabase/migrations/20260526070000_act_score_scales_v2.sql`):** replaced the linear curve with a power-function curve `scaled = round(1 + 35 * (raw/max_raw)^EXPONENT)` clamped `[1, 36]`. Per-section exponents are English/Reading `0.65` and Math/Science `0.70` — concave (exponent < 1) so the middle of the range is more forgiving and matches the typical ACT "raw 50% ≈ scaled 22–23" anchor (linear v1 produced scaled 18 at the same point). Per-section row counts are unchanged (51/46/37/41). **Upgrade path:** drop in a real published table (e.g. from ACT's "Preparing for the ACT" booklets) via a v3 migration — same shape, single SQL file, no code change.

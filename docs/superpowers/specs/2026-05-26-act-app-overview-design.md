...
PK: `(section, raw_score)`. **v1 seed:** a smooth linear-interpolation approximation per section (raw 0 → scaled 1, raw N_max → scaled 36) — sufficient to exercise the round-trip end-to-end and produce a sensible score band; not calibrated to any specific published form. **Upgrade path:** drop in a real published table (e.g. from ACT's "Preparing for the ACT" booklets) via a later migration. Because the seed lives in a versioned migration file, the swap is one SQL file and no code change. Sub-project #5 (Analytics) may revisit calibration once enough attempt data exists.

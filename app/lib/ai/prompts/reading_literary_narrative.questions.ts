...
export function buildReadingLiteraryNarrativeQuestionsPrompt(
  passageBody: string,
  difficulty: Difficulty,
): string {
  const count = PASSAGE_QUESTION_COUNTS.literary_narrative; // 9
  const skills = SKILLS.reading.join(', ');
  return `Generate ${count} ACT Reading questions targeting this Literary Narrative passage.

...
- **Per-cell FLOOR GATES backstop the fillRatio picker.** `planBatches` in [`app/lib/ai/generate.ts`](app/lib/ai/generate.ts) sorts batches by `fillRatio = have / target` ascending and slices to `maxBatches`. The fillRatio picker alone could let individual cells sit empty when other cells happen to be close to their target — e.g. if `english_essay` (target 20) has 4 and `conflicting_viewpoints` (target 8) has 0, both fillRatios are 20%/0%, but if every other type is at 30%+, the picker burns batches on whichever ratio is smallest in turn. Two floor values force the empty cells to the front of the plan:
  ...
  **Values now live in `act.app_config.min_skill_floor` / `act.app_config.min_passage_floor`** (defaults 3 / 1 — same as the previous hardcoded constants). `runGeneration()` reads them at the start of every run and passes them into `planBatches()`. Admins tune at **`/admin/settings`** (the second card, "Question pool minimum floor"); live status of which cells are currently below floor lives at **`/admin/floor-status`** (also surfaced as a stat card + nav tab on `/admin`).
  ...
  The ACT Config Provider sub-workflow (`ghVKwVwYjB7T2vPY`) now fetches `min_skill_floor` / `min_passage_floor` from `act.app_config` at runtime and surfaces them as `minSkillFloor` / `minPassageFloor` on the returned config. One-time manual edit remains on the user's side: the Generator workflow's `Plan Batches` Code node still has hardcoded `SKILL_FLOOR = 3` / `PASSAGE_FLOOR = 1` constants — replace those two lines with `const SKILL_FLOOR = $('Config').first().json.minSkillFloor ?? 3;` and `const PASSAGE_FLOOR = $('Config').first().json.minPassageFloor ?? 1;` (n8n doesn't expose a surgical patch for the 30-node generator, so this must be done in the n8n UI). After that one-time edit, floor changes via `/admin/settings` propagate automatically to n8n on the next hourly run (the Config Provider re-runs at the start of each generator execution). Until that edit lands the Vercel daily cron path is the cheap backstop that picks up any cell the hourly n8n run misses.

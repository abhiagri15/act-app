...
- **Per-cell FLOOR GATES backstop the fillRatio picker.** `planBatches` in [`app/lib/ai/generate.ts`](app/lib/ai/generate.ts) sorts batches by `fillRatio = have / target` ascending and slices to `maxBatches`. The fillRatio picker alone could let individual cells sit empty when other cells happen to be close to their target — e.g. if `english_essay` (target 20) has 4 and `conflicting_viewpoints` (target 8) has 0, both fillRatios are 20%/0%, but if every other type is at 30%+, the picker burns batches on whichever ratio is smallest in turn. Two floor constants force the empty cells to the front of the plan:
  - `SKILL_FLOOR = 3` — every `(math_skill, difficulty)` cell with fewer than 3 enabled questions gets `fillRatio = 0` (top priority).
  - `PASSAGE_FLOOR = 1` — every `passage_type` with fewer than 1 enabled passage gets `fillRatio = 0`.

  Once every cell is at or above the floor, the normal `have / target` sort resumes. Tune in code (no admin UI for v1); the n8n primary path is NOT yet floor-aware — sub-workflow follow-up — but the Vercel daily cron path is, and that's the cheap backstop that picks up any cell the hourly n8n run misses.
